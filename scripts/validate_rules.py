...
vendored engine consumes. This complements the format and guidance guards
with field-level schema validation.
...
* missing ``automated`` metadata: warn on the historical Linux gap; optional
  backfill derives catalog metadata, not proof that a fixer exists or passed.
...
def _expected_automated(rule: dict[str, Any]) -> bool:
    """Derive catalog metadata, not runtime implementation or compliance proof."""
    return rule.get("assessment") == "Automated" and rule.get("risk") != "none"


def validate_rules(data: Any, role: str, problems: list[Problem]) -> None:
    ...
    for idx, rule in enumerate(data):
        rid = str(rule.get("id", f"#{idx}")) if isinstance(rule, dict) else f"#{idx}"
        ...
        if isinstance(rule.get("levels"), list):
            bad = [lv for lv in rule["levels"] if type(lv) is not int or lv not in VALID_LEVELS]
            if bad:
        ...
        if "automated" not in rule:
            ...
            problems.append(Problem(
                "warn", role, rid,
                f"missing 'automated' field (metadata default {expected}); "
                f"run with --write-backfill to add it",
            ))


def _guidance_ids(guidance: Any) -> set[str]:
...
def validate_guidance(guidance: Any, rules_path: Path, role: str, problems: list[Problem]) -> None:
...
def iter_catalogs() -> Iterator[Path]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Summary.
    if not checked:
        print("ERROR: no rule catalogs found")
        return 1
    if errors:
