...
def cmd_verify_image(args: argparse.Namespace, image_id: str | None = None,
    ...
    try:
        ...
        if r.family == "windows":
            ...
            if not ssh_ok:
                # Make the failure diagnosable: surface the VNC console URL
                # instead of a bare timeout line.  --keep-on-fail leaves the
    ...
    *--output json* (roadmap D-95): emit the stable ``list/v1`` contract
        ...
        for name, meta in sorted(PROFILES.items()):
            ...
            if show_versions:
                ...
                entry["rules_sha256"] = (ohbs_image._bundled_rules_hash(role, cat)[:16]
                                         if role else "-")
                entry["engine_version"] = (ohbs_image._engine_version(
                    ohbs_image._engine_path(role)) if role else "-")
            items.append(entry)
    ...
    if show_versions:
        ...
        for name, meta in sorted(PROFILES.items()):
            ...
            ev = ohbs_image._engine_version(ohbs_image._engine_path(role)) if role else "-"
            print(f"{name:<12} {bm:<14} {cat:<22} {rh:<18} {ev}")
        return 0
