...
- verify/cleanup subcommand-group convergence (flat legacy names stay
  registered as deprecated aliases, removal target 1.0.0)
"""
...
class TestDeprecatedEntryAlias:
    def test_cis_image_alias_warns(self, capsys):
        ...
        assert "cis-image" in err and "deprecated" in err and "1.0.0" in err
...
class TestVerifyCleanupGroups:
    ...
    remain registered as deprecated aliases (removal target 1.0.0), and the
...
class TestDeprecatedAliasWrapper:
    ...
    1.0.0 removal target, then dispatches to the real handler."""
