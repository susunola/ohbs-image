...
@pytest.mark.parametrize("levels", [[True], [1.0], [[]], [{}]])
def test_malformed_levels_are_errors_not_crashes(good_tree, levels):
    _write_catalog(good_tree, "cis-rhel9", [dict(GOOD_RULE, levels=levels)])
    rc, output = _run(good_tree)
    assert rc == 1
    assert "invalid value" in output


def test_boolean_page_is_not_an_integer(good_tree):
    _write_catalog(good_tree, "cis-rhel9", [dict(GOOD_RULE, page=True)])
    rc, output = _run(good_tree)
    assert rc == 1
    assert "must be an integer" in output


def test_missing_catalogs_fail_closed(tmp_path):
    rc, output = _run(tmp_path)
    assert rc == 1
    assert "no rule catalogs" in output
